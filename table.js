...
const statConfig = {
  ...
  defensive_contribution: { key: 'defensive_contribution', label: 'DCs', tooltip: 'DEFCON Pts Awarded: 2pts per game with 10+ contributions (DEF) or 12+ (MID/FWD)' }
};
...
function buildFixtureChipsHtml(fixtures) {
  ...
  const chips = upcomingGWs.flatMap(gw => {
    ...
    return gwFxs.map(fx => {
      ...
      return `<div class="pim-fix-chip${isDGW ? ' pim-fix-chip--dgw' : ''}${overrideClass}" title="${overrideTitle}">
...
window.showPlayerInfo = function (ev, playerId) {
  ...
  fetchPlayerSummary(playerId)
    .then(data => {
      ...
      const fixtureChipsHtml = buildFixtureChipsHtml(applyOverrides(data.fixtures || []));
